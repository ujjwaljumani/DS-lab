...
void deleteNumber(int num) {
    ...
    struct Node* temp = head;
    // Search for the node containing the number
...
void reverseList() {
    ...
    while (current != NULL) {
        ...
        current->next = temp;
        // Move to the next node in the original order (which is now current->prev)
        current = current->prev;
    }

    // Before finishing, reset the head to the last node processed
    if (temp != NULL) {
        head = temp->prev;
    }
...
void concatenate() {
    ...
    // Build the second list temporarily
